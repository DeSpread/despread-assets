...
_INIT_STORY_GETH_VERSION="1.2.1"
_INIT_STORY_VERSION="1.5.2"
...
install_story_geth() {
    wget -q $(get_aws_geth_binary_url "$_INIT_STORY_GETH_VERSION") -O /tmp/story-geth
    mkdir -p $HOME/go/bin
...
install_story_binary() {
    wget -q $(get_aws_story_binary_url "$_INIT_STORY_VERSION") -O /tmp/story
    mkdir -p $HOME/.story/story/cosmovisor/genesis/bin
    sudo chmod a+x /tmp/story
    sudo cp /tmp/story $HOME/.story/story/cosmovisor/genesis/bin/story
}
  ...
  while true; do
    ...
    case $upgrade_choice in
        1)
            ...
            read -p "Enter the new version of the client (e.g. v0.13.0): " client_version
